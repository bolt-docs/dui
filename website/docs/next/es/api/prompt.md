...
| `onCancel` | `() => void` | — | Se llama una vez al cancelar el prompt (Escape con query vacía, o Ctrl+C) |
| `colors` | `object` | — | Sobrescritura de colores por llamada |
...
| `children` | `TreeNode<T>[] \| (() => Promise<TreeNode<T>[]>)` | — | Nodos hijo (crea una rama). Pasa una **función** para cargar hijos de forma perezosa — el loader corre una sola vez al primer expandir (con caché) y la fila muestra un indicador `…` mientras carga |

<Callout variant="info">
  **Lazy loading** — dale a una rama `children: async () => [...]` para traer sus
  hijos bajo demanda. El resultado queda en caché, así que los toggles de
  expandir/colapsar son instantáneos tras la primera carga. Ideal para árboles de
  archivos, navegadores de paquetes o jerarquías profundas que no deberían recorrerse
  por adelantado.
</Callout>

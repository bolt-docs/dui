...
| `type` | `"text" \| "password"` | `"text"` | `"password"` masks the value with `•` and never echoes it |
| `colors` | `object` | — | Per-call color overrides |

```ts
const token = await input('GitHub token:', {
  type: 'password',
  validate: (v) => v.length >= 8 ? true : 'Too short',
})
// Renders •••••••• while typing; resolves with the real value
```
...
| `searchable` | `boolean` | `false` | Type to fuzzy-filter (see [Fuzzy Search](/docs/next/api/fuzzy)); mutually exclusive with `enableDragReorder` |
| `onCancel` | `() => void` | — | Called once when the prompt is cancelled (Escape with empty query, or Ctrl+C) — restore terminal state or release resources |
| `colors` | `object` | — | Per-call color overrides |
...
| `pageSize` | `number` | `10` | Max visible items before scrolling |
| `searchable` | `boolean` | `false` | Type to fuzzy-filter (see [Fuzzy Search](/docs/next/api/fuzzy)) |
| `colors` | `object` | — | Per-call color overrides |
...
| `searchable` | `boolean` | `false` | Type to fuzzy-filter the flattened tree (see [Fuzzy Search](/docs/next/api/fuzzy)) |
| `onCancel` | `() => void` | — | Called once when the prompt is cancelled (Escape with empty query, or Ctrl+C) |
| `colors` | `object` | — | Per-call color overrides |
...
| `children` | `TreeNode<T>[] \| (() => Promise<TreeNode<T>[]>)` | — | Child nodes (creates a branch). Pass a **function** to load children lazily — the loader runs once on first expand (cached), the row shows a `…` indicator while it loads |

<Callout variant="info">
  **Lazy loading** — give a branch `children: async () => [...]` to fetch its
  children on demand. The result is cached, so expand/collapse toggles are instant
  after the first load. Great for file trees, package browsers, or any deep hierarchy
  that shouldn't be walked upfront:

  ```ts
  const file = await tree('Pick a file', {
    tree: [{
      label: 'src',
      children: async () => (await listDir('src')).map((f) => ({ label: f, value: f })),
    }],
  })
  ```
</Callout>
